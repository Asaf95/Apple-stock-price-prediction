...
def LSTM_prepare_variables(df):
    ...
    masho.LSTM_model(q_80, q_90, dates_train, X_train, y_train, dates_val, X_val, y_val, dates_test, X_test, y_test)
...
def classification_models(df):
    ...
    y = y1
    result = ml_classification_models.main(X, y)
    print('------')
    y = y2
    result2 =ml_classification_models.main(X, y)
